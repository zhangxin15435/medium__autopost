    ...
    async readCSV(filePath) {
        try {
            ...
            if (fileContent.charCodeAt(0) === 0xFEFF) {
                ...
                logger.info('检测到UTF-8 BOM标记，已移除');
            }
            ...
            return new Promise((resolve, reject) => {
                try {
                    ...
                    bufferStream
                        ...
                        .on('end', () => {
                            logger.info(`CSV解析完成，共 ${articles.length} 行数据`);

                            // 调试输出前几行数据
                            if (articles.length > 0) {
                                logger.info(`CSV标题: ${Object.keys(articles[0]).join(', ')}`);
                                logger.info(`第一行数据: ${JSON.stringify(articles[0])}`);
                            }
                    ...
                    logger.error('CSV流处理错误:', error);
                    reject(error);
                }
            });
        } catch (error) {
            logger.error('CSV文件读取错误:', error);
