...
class BatchPublisher {
    ...
    async previewTable(tableFilePath) {
        try {
            logger.info(`开始预览表格文件: ${tableFilePath}`);

            // 检查文件是否存在
            if (!await fs.pathExists(tableFilePath)) {
                logger.error(`文件不存在: ${tableFilePath}`);
                return {
                    success: false,
                    error: `文件不存在: ${tableFilePath}`
                };
            }

            // 检查文件大小
            const stats = await fs.stat(tableFilePath);
            ...
            try {
                ...
                if (articles.length === 0) {
                    logger.warn('表格中没有找到文章数据');
                    return {
                        success: false,
                        error: '表格中没有找到有效的文章数据'
                    };
                }
                    ...
                    try {
                        return {
                            ...
                            contentPreview: article.content ?
                                (article.content.substring(0, 100) + '...') : '无内容',
                        ...
                        logger.error('处理文章预览数据失败:', err);
                        return {
                            index: article.index || 0,
                            title: '数据处理错误',
                            contentPreview: '无法显示内容',
                            shouldPublish: false,
                            tags: [],
                            hasImage: false,
                            notes: err.message
                ...
                return {
                    ...
                    statistics,
                    data: {
                        rows: previewArticles
                    },
                    articles: previewArticles
                };
            } catch (readError) {
                logger.error('读取表格文件失败:', readError);
                return {
                    success: false,
                    error: `读取表格文件失败: ${readError.message}`
                };
            }
        } catch (error) {
            logger.error('预览表格文件失败:', error);
            return {
                success: false,
                error: `预览表格文件失败: ${error.message}`
            };
        }
    }
